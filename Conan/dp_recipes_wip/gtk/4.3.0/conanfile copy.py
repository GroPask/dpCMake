...
# Modified version of
# https://github.com/conan-io/conan-center-index/blob/master/recipes/gtk/all/conanfile.py
...
class LibnameConan(ConanFile):
    ...
    def build_requirements(self):
        ...
        if self._gtk4:
            self.build_requires("sassc/3.6.1@dp_recipe/stable")
    ...
    def source(self):
        ...
        #if tools.Version(self.version) >= tools.Version("4.3.0"):
        #    extracted_dir = self.name + "-master"
        #if self._gtk3:
        #    extracted_dir = extracted_dir.replace("gtk", "gtk+")
        #if self.settings.os == "Windows":
        #    tools.replace_in_file(extracted_dir + "/tools/meson.build", '''  ['gtk4-query-settings', ['gtk-query-settings.c'], [libgtk_dep]],''', '''  ['gtk4-query-settings', ['gtk-query-settings.c'], [libgtk_static_dep]],''')
        #    tools.replace_in_file(extracted_dir + "/tools/meson.build", '''                         'gtk-builder-tool-preview.c'], [libgtk_dep] ],''', '''                         'gtk-builder-tool-preview.c'], [libgtk_static_dep] ],''')
        #    tools.replace_in_file(extracted_dir + "/meson.build", '''gtk_update_icon_cache: true,''', '''gtk_update_icon_cache: false,''') # TMP
        #if not self.options.shared:
        #    tools.replace_in_file(extracted_dir + "/gtk/gtkwin32.c", '''DllMain (HINSTANCE hinstDLL,''', '''DllMainBis (HINSTANCE hinstDLL,''')
        #    tools.replace_in_file(extracted_dir + "/gtk/meson.build", '''libgtk = shared_library('gtk-4',''', '''libgtk = static_library('gtk-4',''')
    ...
    def package(self):
        ...
        if self.settings.os == "Windows" and not self.options.shared:
            tools.rename(os.path.join(self.package_folder, "lib", "libgtk-4.a"), os.path.join(self.package_folder, "lib", "gtk-4.lib"))
    ...
    def package_info(self):
        if self._gtk3:
            ...
            if self.settings.os == "Windows" and not self.options.shared:
                self.cpp_info.system_libs = ["Dwmapi.lib", "imm32.lib", "winmm.lib", "Ws2_32.lib", "Setupapi.lib", "comctl32.lib", "Crypt32.lib"]